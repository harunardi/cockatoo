...
def main():
    ...
    if geom_type =='1D':
        ...
        DENOM_LAMDA = 0
        for g in range(group):
            NUMER_LAMDA += trapezoid((1/v[g]) * (PHI_ADJ_reshaped[g] * PHI_reshaped[g]), dx=dx, axis = 0)
            DENOM_LAMDA += trapezoid(PHI_ADJ_reshaped[g] * S_DENOM_reshaped[g], dx=dx, axis = 0)
        ...
        # ZPRTF differences
        diff_G0_mag = ((mag_G_0 - abs(G0_analytical)) / abs(G0_analytical)) * 100
        diff_G0_phase = ((phase_G_0 - np.degrees(np.angle(G0_analytical))) / np.degrees(np.angle(G0_analytical))) * 100
        ...
        plt.plot(freq, mag_G_0, marker='o', label="Numerical")
        plt.plot(freq, abs(G0_analytical), label="Analytical")
        plt.xscale('log')
        plt.yscale('log')
        ...
        plt.legend()
        plt.savefig(f'{output_dir}/{case_name}_TRANSFER/{case_name}_{solver_type.upper()}/{case_name}_{solver_type.upper()}_Magnitude_freq.png')

        # Plotting phase
        plt.figure(figsize=(8, 6))  # Create another new figure
        plt.plot(freq, phase_G_0, marker='o', label="Numerical")
        plt.plot(freq, np.degrees(np.angle(G0_analytical)), label="Analytical")
        plt.xscale('log')
        plt.xlabel('Frequency (Hz)')
        plt.ylim(-120, 0)
        ...
        plt.legend()
        plt.savefig(f'{output_dir}/{case_name}_TRANSFER/{case_name}_{solver_type.upper()}/{case_name}_{solver_type.upper()}_Phase_freq.png')

        # Plotting magnitude
        plt.figure(figsize=(8, 6))  # Create a new figure
        plt.plot(omega_plot, mag_G_0, marker='o', label="Numerical")
        plt.plot(omega_plot, abs(G0_analytical), label="Analytical")
        plt.xscale('log')
        plt.yscale('log')
        ...
        plt.title('Plot of Transfer Function (Magnitude)')
        plt.legend()
        plt.savefig(f'{output_dir}/{case_name}_TRANSFER/{case_name}_{solver_type.upper()}/{case_name}_{solver_type.upper()}_Magnitude_omega.png')

        # Plotting phase
        plt.figure(figsize=(8, 6))  # Create another new figure
        plt.plot(omega_plot, phase_G_0, marker='o', label="Numerical")
        plt.plot(omega_plot, np.degrees(np.angle(G0_analytical)), label="Analytical")
        plt.xscale('log')
        plt.xlabel('Omega (Rad/s)')
        plt.ylim(-120, 0)
        plt.ylabel('Phase of Transfer Function')
        plt.title('Plot of Transfer Function (Phase)')
        plt.legend()
        plt.savefig(f'{output_dir}/{case_name}_TRANSFER/{case_name}_{solver_type.upper()}/{case_name}_{solver_type.upper()}_Phase_omega.png')

        # Plotting magnitude
        plt.figure(figsize=(8, 6))  # Create a new figure
        plt.plot(freq, diff_G0_mag, marker='o')
        plt.xscale('log')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Difference of Magnitude of Transfer Function (%)')
        plt.title('Plot of Difference Transfer Function (Magnitude) in %')
        plt.savefig(f'{output_dir}/{case_name}_TRANSFER/{case_name}_{solver_type.upper()}/{case_name}_{solver_type.upper()}_Diff_Magnitude_freq.png')

        # Plotting phase
        plt.figure(figsize=(8, 6))  # Create another new figure
        plt.plot(freq, diff_G0_phase, marker='o')
        plt.xscale('log')
        plt.xlabel('Frequency (Hz)')
        ...
        DENOM_LAMDA = 0
        for g in range(group):
            NUMER_LAMDA += trapezoid((1/v_new[g]) * (PHI_ADJ_temp_reshaped[g] * PHI_temp_reshaped[g]), dx=dx*dy, axis = 0)
            DENOM_LAMDA += trapezoid(PHI_ADJ_temp_reshaped[g] * S_DENOM_reshaped[g], dx=dx*dy, axis = 0)
        Lamda = NUMER_LAMDA / DENOM_LAMDA
        omega_space = 2 * np.pi * freq
        G0_analytical = 1 / ((1j * omega_space) * (Lamda + (Beff / (1j * omega_space + l))))

        # Numerical Solution of ZPRTF 
        G_0 = []
        mag_G_0 = []
        phase_G_0 = []
        for f in range(len(freq)):
        ...
        # Lambda and Analytical Solution of ZPRTF
        PHI_temp_reshaped_new = np.nan_to_num(PHI_temp_reshaped, nan=0)
        PHI_ADJ_temp_reshaped_new = np.nan_to_num(PHI_ADJ_temp_reshaped, nan=0)
        S_DENOM = F_FORWARD.dot(PHI)
        S_DENOM_reshaped = np.reshape(S_DENOM, (group, max(conv_tri)))
        S_DENOM_reshaped_new = np.nan_to_num(S_DENOM_reshaped, nan=0)
        NUMER_LAMDA = 0
        DENOM_LAMDA = 0
        for g in range(group):
            NUMER_LAMDA += trapezoid((1/v_new[g]) * (PHI_ADJ_temp_reshaped_new[g] * PHI_temp_reshaped_new[g]), dx=h**2/4*np.sqrt(3), axis = 0)
            DENOM_LAMDA += trapezoid(PHI_ADJ_temp_reshaped_new[g] * S_DENOM_reshaped_new[g], dx=h**2/4*np.sqrt(3), axis = 0)
        Lamda = NUMER_LAMDA / DENOM_LAMDA
        omega_space = 2 * np.pi * freq
        G0_analytical = 1 / ((1j * omega_space) * (Lamda + (Beff / (1j * omega_space + l))))
        ...
        DENOM_LAMDA = 0
        for g in range(group):
            NUMER_LAMDA += trapezoid((1/v_new[g]) * (PHI_ADJ_temp_reshaped[g] * PHI_temp_reshaped[g]), dx=dx*dy*dz, axis = 0)
            DENOM_LAMDA += trapezoid(PHI_ADJ_temp_reshaped[g] * S_DENOM_reshaped[g], dx=dx*dy*dz, axis = 0)
        Lamda = NUMER_LAMDA / DENOM_LAMDA
        omega_space = 2 * np.pi * freq
        G0_analytical = 1 / ((1j * omega_space) * (Lamda + (Beff / (1j * omega_space + l))))
        ...
        for f in range(len(freq)):
            ...
            print(f"dPOWER = {NUMER/DENOM}")
            dPOWER.append(NUMER / DENOM)
            dPOWER_f = NUMER / DENOM
            ...
            print(f"dRHO = {NUMER/DENOM}")
            dRHO.append(NUMER / DENOM)
            dRHO_f = NUMER / DENOM

            print(f'G_0 = {dPOWER_f/dRHO_f}')

            omega_G0 = 2 * np.pi * ff
            G0_analytical_f = 1 / ((1j * omega_G0) * (Lamda + (Beff / (1j * omega_G0 + l))))
            print(f'G0_analytical = {G0_analytical_f}')
