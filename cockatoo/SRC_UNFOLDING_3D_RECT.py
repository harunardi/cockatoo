...
def main_unfold_3D_rect_greedy(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, K_max, conv, output_dir, case_name, x, y, z):
    max_conv = max(conv)
    conv_array = np.array(conv)
    non_zero_conv = np.nonzero(conv)[0]
    dPHI_temp_conv = conv_array[non_zero_conv] - 1
...
#######################################################################################################
def main_unfold_3D_rect_greedy_new(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, K_max, conv, output_dir, case_name, x, y, z):
    max_conv = max(conv)
    conv_array = np.array(conv)
    non_zero_conv = np.nonzero(conv)[0]
    dPHI_temp_conv = conv_array[non_zero_conv] - 1
